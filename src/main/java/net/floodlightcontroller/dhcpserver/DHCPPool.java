...
public class DHCPPool {
	 ...
	 * null MAC address and the lease is set to available.
	 *
	 ...
	 */
	public Optional<DHCPBinding> getDHCPbindingFromIPv4(@Nonnull IPv4Address ip) {
		return dhcpPool.stream()
	 ...
	 */
	public Optional<DHCPBinding> getDHCPbindingFromMAC(@Nonnull MacAddress mac) {
		return dhcpPool.stream()
				.filter(binding -> binding.getMACAddress().equals(mac))
				.findAny();
	}
	/**
	...
	public void setNormalLease(DHCPBinding binding, @Nonnull MacAddress mac, long time) {
		if (binding.getCurrLeaseState() == LeasingState.AVAILABLE) {
			log.warn("Attempt to set a dhcp binding that is not available");
			return;
		}
	 ...
	 *
	 * After that, this method does not set the lease as leased, but instead reserves that IP
	...
	public void setPermanentLease(DHCPBinding binding, @Nonnull MacAddress mac) {
		if (binding.getCurrLeaseState() != LeasingState.AVAILABLE) {
			log.warn("Attempt to set an lease binding that is not available");
			return;
		}
	 ...
	 * If this is a new client Mac, then the lowest currently available address will be returned.
	 *
	...
	//TODO: refactor getLeaseForClient here -- client side only cares the returned IP address assignment, not the process
	public Optional<IPv4Address> assignIpForClient(@Nonnull MacAddress clientMac, long time) {
		Optional<DHCPBinding> lease = this.getDHCPbindingFromMAC(clientMac);

		// Client registered already
		if (lease.isPresent()) {
			switch (lease.get().getCurrLeaseState()) {
				case AVAILABLE:
					log.error("We have a registered client MAC but lease still mark as available, this should never happen!");
					break;

				case LEASED:
					return Optional.of(lease.get().getIPv4Address());
					break;

				case PERMANENT_LEASED:
					return Optional.of(lease.get().getIPv4Address());
					break;
			...
			Optional<DHCPBinding> newLease = dhcpPool.stream()
								.filter(binding -> binding.getCurrLeaseState() == LeasingState.AVAILABLE)
								.findAny();
	...
	public void displayDHCPPool() {
		if (getPoolSize() > 0) {
			for(DHCPBinding binding : dhcpPool) {
				log.info("Current DHCP pool is {}", binding.toString());
			}
		}
		else {
			log.error("DHCP pool size isn't allocate correctly");
		}
		System.out.println(dhcpPool.toString());
	}
	/**
	...
	public boolean renewLeaseOfMac(@Nonnull MacAddress mac, int time) {
		Optional<DHCPBinding> lease = this.getDHCPbindingFromMAC(mac);
		if (lease.isPresent()) {
