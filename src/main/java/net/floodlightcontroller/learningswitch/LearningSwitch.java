...
public class LearningSwitch implements IFloodlightModule,
		ILearningSwitchService, IOFMessageListener {
	 ...
	 * @return
	 */
	private OFFlowMod writeFlowMod(IOFSwitch sw, OFFlowModCommand command,
	...
	private void writePacketOutForPacketIn(IOFSwitch sw,
		...
		OFMessageUtils.writePacketOutForPacketIn(sw, packetInMessage,
				egressPort);
		counterPacketOut.increment();
	}
	 ...
	 * @return
	 */
	private Command processPacketInMessage(final IOFSwitch sw, OFPacketIn pi,
			FloodlightContext cntx) {
		...
		if (outPort == null) {
			...
			OFBundle bundle = new OFBundle(sw, OFBundle.NO_FLAGS);

			OFFlowMod fm = this.writeFlowMod(sw, OFFlowModCommand.ADD,
					OFBufferId.NO_BUFFER, m, outPort);
			...
			if (LEARNING_SWITCH_REVERSE_FLOW) {
				...
				fm = this.writeFlowMod(sw, OFFlowModCommand.ADD,
						OFBufferId.NO_BUFFER, mb.build(), inPort);

				bundle.add(fm);
			}
			bundle.closeAndCommit();

			// push packet after commit only
