...
public class old_LearningSwitch
implements IFloodlightModule, ILearningSwitchService, IOFMessageListener {
	protected static Logger log = LoggerFactory.getLogger(old_LearningSwitch.class);
	...
	private void writeFlowMod(IOFSwitch sw, OFFlowModCommand command, OFBufferId bufferId,
		...
		fmb.setMatch(match);
		fmb.setCookie((U64.of(old_LearningSwitch.LEARNING_SWITCH_COOKIE)));
		fmb.setIdleTimeout(old_LearningSwitch.FLOWMOD_DEFAULT_IDLE_TIMEOUT);
		fmb.setHardTimeout(old_LearningSwitch.FLOWMOD_DEFAULT_HARD_TIMEOUT);
		fmb.setPriority(old_LearningSwitch.FLOWMOD_PRIORITY);
	...
	private Command processFlowRemovedMessage(IOFSwitch sw, OFFlowRemoved flowRemovedMessage) {
		if (!flowRemovedMessage.getCookie().equals(U64.of(old_LearningSwitch.LEARNING_SWITCH_COOKIE))) {
			return Command.CONTINUE;
		}
		if (log.isTraceEnabled()) {
