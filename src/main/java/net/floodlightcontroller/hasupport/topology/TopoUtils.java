 ...
 * used for repetitive tasks by different topology classes Specifically consists
 * of functions to parse the Topology updates and calculating md5 hashes,
 * updates are parsed in O(n) time to get the equivalent JSON representation.
 * 
...
public class TopoUtils {
	private static final Logger logger = LoggerFactory.getLogger(TopoUtils.class);
	 ...
	 *
	 * @param chunk:
	 *            Continuous String representation of an array of Topology
	 *            Updates.
	 * @return List of JSON representation of the Topology Updates which were
	 *         input.
	 */
	...
	public List<String> parseChunk(String chunk) {
		...
		try {
			while (!chunk.equals("]]")) {
				...
				// logger.debug("\n[Assemble Topo Update] Chunk pre: {}", new
				// Object[] {chunk});
				...
				if (chunk.startsWith("operation=")) {
					chunk = chunk.substring(10, chunk.length());
					op = chunk.split(",|]")[0];
					// logger.debug("[Assemble Topo Update] Operation=: {}", new
					// Object[]{op});
				...
				// logger.debug("\n[Assemble Topo Update] Chunk keywords: {}",
				// new Object[] {chunk});
				...
				if (chunk.startsWith("src=")) {
					...
					src = chunk.split(",|]")[0];
					// logger.debug("[Assemble Topo Update] Src=: {}", new
					// Object[]{src});
				...
				if (chunk.startsWith("srcPort=")) {
					...
					// logger.debug("[Assemble Topo Update] SrcPort=: {}", new
					// Object[]{srcPort});
				...
				if (chunk.startsWith("dst=")) {
					chunk = chunk.substring(4, chunk.length());
					dst = chunk.split(",|]")[0];
					// logger.debug("[Assemble Topo Update] Dst=: {}", new
					// Object[]{dst});
				...
				if (chunk.startsWith("dstPort=")) {
					...
					// logger.debug("[Assemble Topo Update] DstPort=: {}", new
					// Object[]{dstPort});
				...
				if (chunk.startsWith("latency=")) {
					...
					// logger.debug("[Assemble Topo Update] Latency=: {}", new
					// Object[]{latency});
				...
				if (chunk.startsWith("type=")) {
					...
					type = chunk.split(",|]")[0];
					// logger.debug("[Assemble Topo Update] Type=: {}", new
					// Object[]{type});
				...
				// logger.debug("\n[Assemble Topo Update] Chunk post: {}", new
				// Object[] {chunk});
