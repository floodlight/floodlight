...
public class PortDownReconciliation implements IFloodlightModule,
	...
	public String getName() {
		return "Port Down Reconciliation";
	}
	...
	public List<OFFlowStatisticsReply> getFlows(IOFSwitch sw, Short outPort) {
		...
		// The list of flows that have the specified output action outport we specified
		List<OFFlowStatisticsReply> flowsWithOutport = new ArrayList<OFFlowStatisticsReply>();
		// The list of all the flows which we will have filtered
		List<OFFlowStatisticsReply> flows = null;

		// Try to get the flows
		try {
			flows = sw.getFlows();
		} catch (Exception e) {
			log.debug("Switch " + sw.getId() + " has no flows");
		}

		// Filter the flows and add those with the specified output action
		// outport to the other list
		if (flows != null) {
			for (OFFlowStatisticsReply flow : flows) {
				// Make sure the flow is a forwarding flow
				if (flow.getActions().get(0).getType()
						.equals(OFActionType.OUTPUT)) {
					OFActionOutput action = (OFActionOutput) flow.getActions()
							.get(0);
					// Verify that the Output Action outPort matches the one we seek
					if (action.getPort() == outPort) {
						// Add it to the list
						flowsWithOutport.add(flow);
					}
				}
			}
		}
		return flowsWithOutport;
	}
