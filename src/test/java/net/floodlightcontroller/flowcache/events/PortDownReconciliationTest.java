...
*    under the License.
**/
package net.floodlightcontroller.flowcache.events;
...
/**
 * Unit test for PortDownReconciliation. To test the class I have generated there very simple network topologies.
 * an OFMatchReconcile object with information about the PORT_DOWN event is passed to the class, where it begins 
 * breaking down the information,analyzing the switches for flows and deleting those that are invalid. This Test 
 * specifically verifies that each switch is queried for flows once and results in the appropriate Flow Mod delete
 * message.
 * 
...
public class PortDownReconciliationTest extends FloodlightTestCase {
	    ...
	    protected List <OFFlowStatisticsReply> flows;
	    protected OFMatchReconcile ofmr;
	    protected OFFlowStatisticsReply flow;
    ...
    public void setUp() throws Exception {
        ...
        // Create the flows we want to sit on the switches
        flows = new ArrayList<OFFlowStatisticsReply>(); 
        
        // Actions
        flow = new OFFlowStatisticsReply();
        ...
        m.setDataLayerDestination("00:00:00:00:00:01");
        m.setDataLayerSource("00:00:00:00:00:02");
        // Set the incoming port to 1 so that it will find the connected switches
        ...
        // Expect that reconcileFlows will query the switch for flows once
    	expect(sw1.getFlows()).andReturn(flows).once();
        ...
        // Expect that reconcileFlows will query the switch for flows once
    	expect(sw2.getFlows()).andReturn(flows).once();
        ...
        // Expect that reconcileFlows will query the switch for flows once
    	expect(sw3.getFlows()).andReturn(flows).once();
        ...
        // Expect that reconcileFlows will query the switch for flows once
    	expect(sw4.getFlows()).andReturn(flows).once();
		...
		OFMatchWithSwDpid ofmatchsw = new OFMatchWithSwDpid(match, affectedSwitch);
	...
	/**
	 * This tests the port down reconciliation in the event that the base switch is the only 
	 * switch involved in the PORT_DOWN event. It simply deletes flows concerning the downed port.
	 * @verify checks to see that a general clearFlowMods(Short outPort) is called
	 * @throws Exception
	 */
	@Test
	...
	/**
	 * This tests the port down reconciliation in the event that the base switch is connected
	 * to a chain of three switches. It discovers that is has 1 neighbor, which recursively finds out
	 * that it has 1 neighbor until the final switch "sw4" is evaluated, which has no neighbors.
	 * @verify checks to see that a general clearFlowMods(Short outPort) is called on the base switch
	 * while specific clearFlowMods(OFMatch match, Short outPort) are called on the neighboring switches
	 * @throws Exception
	 */
	@Test
	public void testLinearLinkPortDownReconciliation() throws Exception{
		...
		// Make sure each capture has captured the proper Flow Mod Delete message
	...
	/**
	 * This tests the port down reconciliation in the event that the base switch has 
	 * three separate neighboring switches with invalid flows. It discovers that is has 
	 * 3 neighbors and each of them delete flows with the specific OFMatch and outPort.
	 * @verify checks to see that a general clearFlowMods(Short outPort) is called on the base switch
	 * while specific clearFlowMods(OFMatch match, Short outPort) are called on the neighboring switches
	 * @throws Exception
	 */
