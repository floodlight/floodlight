...
if args.action=='addVipPool':
	command = "curl -X POST -d '{\"id\": \"%s\", \"name\":\"%s\", \"protocol\":\"%s\",\"address\":\"%s\"}' http://localhost:8080/quantum/v1.0/vips/" % (args.id,"vip" + args.id,args.proto,args.addr)
	result = os.popen(command).read()
	print result

	command = "curl -X POST -d '{\"id\": \"%s\", \"name\":\"%s\", \"protocol\":\"%s\", \"lb_method\":\"%s\",\"vip_id\":\"%s\"}' http://localhost:8080/quantum/v1.0/pools/" % (args.id,"pool" + args.id,args.proto,args.lbMethod,args.id)
	result = os.popen(command).read()
	print result

elif args.action=='addMemberToPool':
	command = "curl -X POST -d '{\"id\": \"%s\", \"address\":\"%s\", \"pool_id\":\"%s\",\"weight\":\"%s\"}' http://localhost:8080/quantum/v1.0/members/" % (args.id,args.addr,args.poolId,args.weight)
	result = os.popen(command).read()
	print result

elif args.action=='addMonitorToPool':
	command = "curl -X POST -d '{\"id\": \"%s\", \"name\":\"%s\",\"pool_id\":\"%s\"}' http://localhost:8080/quantum/v1.0/pools/%s/health_monitors" % (args.id,"monitor"+ args.id,args.poolId,args.poolId)
	result = os.popen(command).read()
	print result

elif args.action=='deleteVipPool':
	command = "curl -X DELETE http://localhost:8080/quantum/v1.0/vips/%s" % (args.id)
	result = os.popen(command).read()
	print result

	command = "curl -X DELETE http://localhost:8080/quantum/v1.0/pools/%s" % (args.id)
	result = os.popen(command).read()
	print result

elif args.action=='deleteMember':
	command = "curl -X DELETE http://localhost:8080/quantum/v1.0/members/%s" % (args.id)
	result = os.popen(command).read()
	print result

elif args.action=='deleteMonitor':
	command = "curl -X DELETE http://localhost:8080/quantum/v1.0/health_monitors/%s" % (args.id)
	result = os.popen(command).read()
	print result

elif args.action=='updateMemberWeight':
	command = "curl -X POST http://localhost:8080/quantum/v1.0/members/%s/%s" % (args.id, args.weight)
	result = os.popen(command).read()
	print result

elif args.action=='setPriorityToMember':
	command = "curl -X POST http://localhost:8080/quantum/v1.0/pools/%s/members/%s" % (args.poolId, args.id)
	result = os.popen(command).read()
	print result

elif args.action=='listVips':
	command = "curl -X GET http://localhost:8080/quantum/v1.0/vips/"
	result = os.popen(command).read()
	print result

elif args.action=='listPools':
	command = "curl -X GET http://localhost:8080/quantum/v1.0/pools/"
	result = os.popen(command).read()
	print result

elif args.action=='listMembers':
	command = "curl -X GET http://localhost:8080/quantum/v1.0/members/"
	result = os.popen(command).read()
	print result

elif args.action=='listMonitors':
	command = "curl -X GET http://localhost:8080/quantum/v1.0/health_monitors/"
	result = os.popen(command).read()
	print result

elif args.action=='enableStatistics':
	command = "curl -X POST http://localhost:8080/wm/statistics/config/enable/json"
	result = os.popen(command).read()
	print result

elif args.action=='disableStatistics':
	command = "curl -X POST http://localhost:8080/wm/statistics/config/disable/json"
	result = os.popen(command).read()
	print result

elif args.action=='listSwitchBandwidth':
	command = "curl -X GET http://localhost:8080/wm/statistics/bandwidth/%s/%s/json" % (args.dpid,args.port)
	result = os.popen(command).read()
	print result

elif args.action=='listPoolStatistics':
	command = "curl -X GET http://localhost:8080/quantum/v1.0/pools/%s/stats" % (args.poolId)
	result = os.popen(command).read()
	print result

elif args.action=='listFlowStatistics':
	command = "curl -X GET http://localhost:8080/wm/statistics/flow/%s/json" % (args.dpid)
	result = os.popen(command).read()
	print result